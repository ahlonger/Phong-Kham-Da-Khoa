...
const XemLichHen = () => {
  ...
  useEffect(() => {
    ...
    // Gọi API lấy toàn bộ lịch hẹn và lọc theo email người đăng nhập
    (async () => {
      try {
        const res = await Api.get("booking"); // => http://localhost:3000/api/booking
        let list = Array.isArray(res.data) ? res.data : [];

        // ⚡ Chỉ lấy lịch hẹn của người đăng nhập hiện tại
        list = list.filter((b) => b.email === storedUser.email);
